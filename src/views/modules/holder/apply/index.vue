<template>
  <div>
    ...
    <el-table ref="singleTable" :data="dataList" border @current-change="handleCurrentChange"
      ...
      <el-table-column prop="attrs" label="证书属性" min-width="100"></el-table-column>
      <el-table-column prop="issuerName" label="发证机构" min-width="100"></el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取数据列表
      getDataList() {
        let result = this.$apis.$holder.getApplyCertList(this.dataForm.certStatus, this.dataForm.searchKey, this.pageIndex, this.pageSize);
        if (result) {
